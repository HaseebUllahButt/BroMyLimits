...
const SEAL_AFTER_MS = 7 * 24 * 60 * 60_000;
 ...
 */
const DEDUPE_RING = 32;

/** The six token counters every provider is folded into. */
const FIELDS = ['input', 'output', 'cacheWrite', 'cacheRead', 'reasoning', 'total'];

const blankTokens = () => ({ input: 0, output: 0, cacheWrite: 0, cacheRead: 0, reasoning: 0, total: 0 });
 ...
 */
async function foldLinesFrom(filePath, startByte, onLine) {
  let fh;
...
async function loadIndex(file, caches) {
...
async function saveIndex(file, caches) {
...
module.exports = {
  FileRollupCache, ResultCache, foldLinesFrom, loadIndex, saveIndex,
  blankStats, addStats, blankTokens,
  SEAL_AFTER_MS, DEDUPE_RING, INDEX_VERSION, FIELDS,
};
